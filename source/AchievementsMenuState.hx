...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if MODS_ALLOWED Mods.setDirectoryFromWeek(); #end
		...
		#if TOUCH_CONTROLS addMobilePad("FULL", "A_B_C"); #end
		super.create();
	...
	override function update(elapsed:Float) {
		if(!goingBack && options.length > 1)
		{
			...
			if(controls.RESET && (options[curSelected].unlocked || options[curSelected].curProgress > 0) #if TOUCH_CONTROLS || mobilePad.buttonC.pressed && (options[curSelected].unlocked || options[curSelected].curProgress > 0) #end)
			{
