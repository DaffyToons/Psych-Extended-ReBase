...
import WeekData;
import FreeplayShape.AudioDisplay; //this will be separated later
...
class MainMenuStateNOVA extends MusicBeatState
{
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			if (usingMouse && canClick)
			{
				if (!FlxG.mouse.overlaps(spr)) {
					...
					#if TOUCH_CONTROLS && !FlxG.mouse.overlaps(mobilePad.buttonA) #end){
	...
	function updateGitAction(callback:({ status: String, conclusion: String } -> Void)):Void {
		try {
			...
			var http = new haxe.Http("https://api.github.com/repos/PsychExtendedThings/Psych-Extended-ReBase/actions/runs?per_page=1");
